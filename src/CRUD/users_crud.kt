...
fun main(args:Array<String>) {
    ...
    //insert query-create
  val res = connection.createStatement().executeUpdate("insert into Users1(id,name,age,email,phone,city,state,country,pincode) values(5,'Alice',26,'alice@gmail.com','9807654545','kannur','Kerala','India',677041)")
    if (res > 0) {
    ...
    //update table
     val res_update=connection.createStatement().executeUpdate("update Users1 set name='Julie',email='julie@gmail.com' where id=3")
    if(res_update > 0){
    ...
    val delete_res=connection.createStatement().executeUpdate("delete from Users1 where id=5")
    if(delete_res > 0){
